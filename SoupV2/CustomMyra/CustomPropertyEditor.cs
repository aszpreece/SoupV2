...
{
	public class CustomPropertyGrid : SingleItemContainer<Grid>
	{
		...
		private class SubGrid : SingleItemContainer<Grid>
		{
			...
			private readonly CustomPropertyGrid _propertyGrid;
			...
			public CustomPropertyGrid PropertyGrid
			{
			...
			public SubGrid(CustomPropertyGrid parent, object value, string header, string category, string filter, Record parentProperty)
			{
				...
				_propertyGrid = new CustomPropertyGrid(parent.PropertyGridStyle, category, parentProperty, parent)
				{
					Object = value,
					Filter = filter,
		...
		private readonly CustomPropertyGrid _parentGrid;
		...
		private CustomPropertyGrid(TreeStyle style, string category, Record parentProperty, CustomPropertyGrid parentGrid = null)
		{
		...
		public CustomPropertyGrid(TreeStyle style, string category) : this(style, category, null)
		{
		}

		public CustomPropertyGrid(string category) : this(Stylesheet.Current.TreeStyle, category)
		{
		}

		public CustomPropertyGrid() : this(DefaultCategoryName)
		{
		}
		...
		private void GetValue(Record record, object obj)
		{
			record.GetValue(obj);
		}
			...
			if (hasSetter)
			{
				spinButton.ValueChanged += (sender, args) =>
				{
					try
					{
						...
						spinButton.Value = (float?)record.GetValue(_object);
					}
		...
		private Grid CreateCollectionEditor(Record record, Type itemType)
		{
			...
			button.Click += (sender, args) =>
			{
				var collectionEditor = new CustomCollectionEditor(items, itemType);
